  ...
  return (
    <div className="space-y-4">
      <AdminHeader
        ...
        actions={(
          <Button asChild><Link href={`/${locale}/admin/events/new`}>Create Event</Link></Button>
        )}
      />
          ...
          <tbody>
            {items.map((n: any) => (
              <tr key={n.id} className="border-t">
                ...
                <td className="p-2">{n.starts_at ? new Date(n.starts_at).toLocaleString() : (n.ends_at ? new Date(n.ends_at).toLocaleString() : "-")}</td>
                <td className="p-2">
                  {n.published_at ? (
