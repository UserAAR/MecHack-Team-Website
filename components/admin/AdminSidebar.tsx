...
import { LayoutDashboard, Newspaper, FolderKanban, CalendarDays, Images, Users2, Settings } from "lucide-react";
...
export function AdminSidebar({ locale, role }: { locale: string; role: AdminRole }) {
  ...
  return (
    <aside className="hidden lg:flex h-full w-72 shrink-0 border-r border-white/10 bg-[linear-gradient(180deg,#0b1024,#0a112a_30%,#0b1024)] text-white">
      <div className="flex flex-col w-full">
        <div className="px-4 py-5">
          <div className="text-lg font-semibold tracking-wide">Qarabag Admin</div>
          <div className="text-[11px] opacity-60">Content Console</div>
        </div>
        <nav className="px-2 pb-6 text-[13px]">
          <div className="px-3 py-2 text-[10px] uppercase tracking-wider/3 opacity-60">Overview</div>
          ...
          {role === "superadmin" ? (
            <div className="mt-5">
              <div className="px-3 py-2 text-[10px] uppercase tracking-wider/3 opacity-60">Administration</div>
              <ul className="space-y-1">
                {superItems.map((it) => {
                  const Icon = it.icon;
                  const active = pathname === it.href || pathname?.startsWith(it.href + "/");
